...
template<class T> const integer_pow_functions_constants<T>::T pow_mod_type_cast_limit = int_sqrt<T>(std::numeric_limits<T>::max()) + 1;
...
template<class B, class E, class R = B, class C = R, bool POW_INT_SAFETY = true> R pow_int(B base, E exponent) {
    ...
    if(POW_INT_SAFETY && std::numeric_limits<E>::is_signed) {
        if(exponent < 0) {
...
template<class B, class E, class R = B, class C = void, bool POW_INT_MOD_SAFETY = true> R pow_int_mod(B base, E exponent, B mod) {
    static_assert(std::numeric_limits<B>::is_integer);
    static_assert(std::numeric_limits<E>::is_integer);
    static_assert(std::numeric_limits<R>::is_integer);
    static_assert(std::is_same<C, void>::value || std::numeric_limits<C>::is_integer);
    if(POW_INT_MOD_SAFETY && std::numeric_limits<B>::is_signed) {
        if(base < 0) {
            throw "pow_int_mod<B, E, R, C>(B base, E exponent, B mod): negative base";
        }
    }
    if(POW_INT_MOD_SAFETY && std::numeric_limits<E>::is_signed) {
        if(exponent < 0) {
    ...
    if(std::is_same<C, void>::value && std::numeric_limits<B>::is_bounded && (std::numeric_limits<B>::digits <= 32) && std::is_same<B, R>) {
    ...
    } else if(std::is_same<C, void>::value && std::numeric_limits<B>::is_bounded && (std::numeric_limits<B>::digits <= 32) && std::is_same<R, uint64_t>) {
        if(mod <= integer_pow_functions_constants<B>::pow_mod_type_cast_limit) {
            R result = 1;
            base %= mod;
            while(exponent) {
    ...
    } else if(std::is_same<C, void>::value && std::numeric_limits<B>::is_bounded && (std::numeric_limits<B>::digits <= 32) && !(std::is_same<B, R>)) {
        if(mod <= integer_pow_functions_constants<B>::pow_mod_type_cast_limit) {
            B result = 1;
            base %= mod;
            while(exponent) {
    ...
    } else if(std::is_same<C, void>::value && std::numeric_limits<B>::is_bounded && (std::numeric_limits<B>::digits > 32) && std::is_same<B, R>::value) {
        if(mod <= integer_pow_functions_constants<B>::pow_mod_type_cast_limit) {
            R result = 1;
            base %= mod;
            while(exponent) {
    ...
    } else if(std::is_same<C, void>::value && std::numeric_limits<B>::is_bounded && (std::numeric_limits<B>::digits > 32) && std::is_same<R, BOOST_UINT(((std::numeric_limits<B>::digits + 1) >> 1) << 2)>) {
        if(mod <= integer_pow_functions_constants<B>::pow_mod_type_cast_limit) {
            B result = 1;
            base %= mod;
            while(exponent) {
        ...
        return static_cast<R>(result);
    } else if(std::is_same<C, void>::value && !(std::numeric_limits<B>::is_bounded) && !(std::is_same<B, cpp_int>::value) && std::is_same<R, cpp_int>::value) {
        R result = 1;
        R base_cast = static_cast<R>(base);
        R mod_cast = static_cast<R>(mod);
        while(exponent) {
